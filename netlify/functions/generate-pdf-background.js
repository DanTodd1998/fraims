...
exports.handler = async (event) => {
  ...
  // The AI draft, if one exists, is now an OPTIONAL overlay - never required.
  const draft =
    assessment.generatedReport && assessment.generatedReport.draft
      ? assessment.generatedReport.draft
      : {};
  const hasAiDraft = draft && Object.keys(draft).length > 0;
  ...
  try {
    ...
    // Manual narrative lives in findings.sectionDrafts, keyed by the UI section
    // name. Report-level manual fields (executiveSummary, limitations) live at
    // the top level of findings.
    const findings = assessment.findings || {};
    const sectionDrafts =
      findings.sectionDrafts && typeof findings.sectionDrafts === "object"
        ? findings.sectionDrafts
        : {};

    // Resolve a section's text: MANUAL WINS, AI draft is the fallback.
    const resolveSection = (manualName, aiKey) => {
      const manual = manualName ? sectionDrafts[manualName] : "";
      if (manual && String(manual).trim()) return String(manual);
      const ai = aiKey ? draft[aiKey] : "";
      if (ai && String(ai).trim()) return String(ai);
      return "";
    };
    ...
    const execSummary =
      findings.executiveSummary && String(findings.executiveSummary).trim()
        ? String(findings.executiveSummary)
        : "";
    const execContent = [];
    if (execSummary) {
      execContent.push({ text: "Executive Summary", fontSize: 14, bold: true, color: LK_NAVY, margin: [0, 12, 0, 4] });
      execContent.push({ text: execSummary, fontSize: 10, margin: [0, 0, 0, 6], lineHeight: 1.3 });
    }

    // ---- Narrative sections ----
    // Each entry: [ aiKey, PDF label, UI section name for the manual field ].
    // Manual text (findings.sectionDrafts[uiName]) wins; draft[aiKey] is fallback.
    const sectionOrder = [
      ["scopeResponsiblePersons", "Scope & Responsible Persons", "Scope & Responsible Persons"],
      ["premisesOccupancy", "Premises & Occupancy", "Premises & Occupancy"],
      ["fireHazards", "Fire Hazards", "Fire Hazards \u2014 Ignition, Fuel, Oxygen"],
      ["meansOfEscape", "Means of Escape", "Means of Escape"],
      ["fireDetectionWarning", "Fire Detection & Warning", "Fire Detection & Warning"],
      ["emergencyLightingSignage", "Emergency Lighting & Signage", "Emergency Lighting & Signage"],
      ["firefightingEquipment", "Firefighting Equipment", "Firefighting Equipment"],
      ["passiveFireProtection", "Passive Fire Protection", "Passive Fire Protection"],
      ["firefighterAccessFacilities", "Firefighter Access & Facilities", "Firefighter Access & Facilities"],
      ["managementTestingRecords", "Management, Testing & Records", "Management, Testing & Records"],
      ["conclusions", "Conclusions", "Conclusions"],
      ["limitations", "Limitations", null],
    ];

    const narrative = [];
    sectionOrder.forEach(([aiKey, label, manualName]) => {
      let val = resolveSection(manualName, aiKey);
      // Limitations has no findings section; its manual text is on the Report
      // Details card at findings.limitations.
      if (aiKey === "limitations") {
        const manualLim =
          findings.limitations && String(findings.limitations).trim()
            ? String(findings.limitations)
            : "";
        val = manualLim || (draft.limitations && String(draft.limitations).trim() ? String(draft.limitations) : "");
      }
    ...
    // Map AI observations by photoId (only present when an AI draft exists).
    const aiById = {};
    ...
    const footerNote = hasAiDraft
      ? "Fire Risk Assessment \u2014 AI-assisted draft"
    ...
    const docDefinition = {
      ...
      footer: (currentPage, pageCount) => ({
        columns: [
          { text: footerNote, fontSize: 8, color: "#888", margin: [40, 0, 0, 0] },
